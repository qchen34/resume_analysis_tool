...
DEBATE_SYSTEM_PROMPT = (
    ...
    "6）你这次发言的**产出**必须覆盖：\n"
    "   - 针对该 JD，候选人的**长处**（可投的理由）与**短板**（可能卡点）；\n"
    "   - **投递与沟通需要注意什么**（渠道/节奏/策略/筛选点）；\n"
    "   - **有什么可以修改/优化**（简历/作品集/话术/面试准备），以及预期收益；\n"
    "   - **有什么不能修改（红线原则）**：例如虚构经历、夸大成果、谎报入职/离职时间、伪造学历等；\n"
    "     对这些红线差距，给出**侧面战术**：如何用真实经历的表述、补证据、选岗位/选公司、调整沟通顺序来降低风险；\n"
    "   - 给出一段可直接复制的 **HR 打招呼语**（50-120 字，贴合该 JD 与候选人真实经历）。\n"
    "JSON 结构：\n"
    "{\n"
    ...
    '  \"strengths\": [\"长处要点 1\", \"长处要点 2\"],\n'
    '  \"gaps\": [\"短板/风险 1\", \"短板/风险 2\"],\n'
    '  \"application_notes\": [\"投递/沟通注意事项 1\", \"注意事项 2\"],\n'
    '  \"what_to_improve\": [\"可改进项 1（简历/作品集/话术/准备）\", \"可改进项 2\"],\n'
    '  \"red_lines\": [\"不可修改的红线 1\", \"红线 2\"],\n'
    '  \"tactics_for_red_lines\": [\"侧面战术 1\", \"侧面战术 2\"],\n'
    '  \"hr_message\": \"一段可直接复制的 HR 打招呼语（50-120 字）\"\n'
    "}\n"
)
...
DEBATE_SUMMARY_SYSTEM_PROMPT = (
    ...
    "4）明确输出：候选人**可改与不可改**（红线）分别是什么，以及对红线差距的**替代战术**；\n"
    "5）加入 ATS（HR 系统）视角分析：基于常见筛选规则，判断该简历在关键词匹配、硬性门槛（年限/学历/行业/岗位相关性）上的通过风险，给出预估通过率与主要卡点；\n"
    "6）加入第一面面试官首屏看简历的视角：对方第一眼可能看到的亮点、疑虑点，以及最可能追问的 3-5 个问题；\n"
    "7）给出一段「最终推荐打招呼语」（可直接复制）。\n"
    ...
    '  \"what_to_improve\": [\"可改进项 1\", \"可改进项 2\"],\n'
    '  \"red_lines\": [\"不可修改的红线 1\", \"红线 2\"],\n'
    '  \"tactics_for_red_lines\": [\"侧面战术 1\", \"侧面战术 2\"],\n'
    '  \"ats_analysis\": {\n'
    '    \"estimated_pass_probability\": \"0-100% 的预估通过率（例如 62%）\",\n'
    '    \"keyword_match_level\": \"高/中/低\",\n'
    '    \"hard_requirements_check\": [\"硬性条件检查结论 1\", \"结论 2\"],\n'
    '    \"likely_rejection_reasons\": [\"可能被系统或 HR 初筛刷掉的原因 1\", \"原因 2\"],\n'
    '    \"ats_optimization_actions\": [\"提高 ATS 通过率的改写动作 1\", \"动作 2\"]\n'
    '  },\n'
    '  \"first_interviewer_impression\": {\n'
    '    \"likely_first_impression\": \"一面面试官 10-30 秒扫读后的总体印象\",\n'
    '    \"perceived_strengths\": [\"第一眼加分点 1\", \"加分点 2\"],\n'
    '    \"perceived_concerns\": [\"第一眼疑虑点 1\", \"疑虑点 2\"],\n'
    '    \"likely_followup_questions\": [\"最可能追问的问题 1\", \"问题 2\", \"问题 3\"]\n'
    '  },\n'
    '  \"recommended_hr_message\": \"最终推荐打招呼语（50-120 字，可直接复制）\"\n'
    "}\n"
)

STRATEGY_SUMMARY_SYSTEM_PROMPT = (
    "你是一名求职与投递策略顾问。用户会提供一段时间内的投递数据统计（如投递数、回复数、面试数、Offer 数及比率）。\n"
    "你需要基于这些数据，用简洁的中文给出：\n"
    "1）当前投递策略的简要评估（例如：回复率正常/偏低/偏高，可能原因）；\n"
    "2）3-5 条可执行的优化建议（如：调整投递渠道、优化简历关键词、聚焦某类岗位/行业、改进打招呼话术等）；\n"
    "3）若用户还提供了历史分析结论摘要，可结合「候选人-岗位匹配」情况给出更针对性的建议。\n"
    "直接输出正文即可，无需 JSON；分点清晰、语气务实。"
)
